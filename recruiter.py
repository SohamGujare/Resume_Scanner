...
def process_resumes(job_description, resume_texts):
    ...
    for resume in resume_texts:
        analysis = advanced_resume_analysis(job_description, resume['text'], resume['filename'])
...
def recruiter_dashboard():
    """Recruiter functionality - multiple resume analysis"""
    if request.method == 'POST':
        ...
        for file in resume_files:
            if file.filename:
                filepath = os.path.join(current_app.config['UPLOAD_FOLDER'], file.filename)  # Use current_app
                file.save(filepath)
